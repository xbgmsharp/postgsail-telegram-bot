...
class API:
    ...
    async def token(self, user_id: int) -> Any:
    ...
    async def otp(self, email: str) -> Any:
        """Generate otp token."""
        async with aiohttp.ClientSession() as session:
            response = await API.post(self, "/rpc/telegram_otp_fn", session, {"email": email})
            print(response)
        await session.close()
        print(response)
        return response if response else None

    async def otp_validation(self, payload) -> Any:
